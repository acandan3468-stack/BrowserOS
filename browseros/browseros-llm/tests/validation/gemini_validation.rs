/// Gemini Provider — Phase 5A Operational Validation
///
/// Validates against a deterministic mock Gemini API server.
...
fn gemini_config(port: u16) -> ProviderConfig {
    ProviderConfig {
        ...
        default_model: Some("gemini-1.5-flash".into()),
...
#[test]
fn s1_chat_basic() {
    let _s = MockServer::start("gemini");
    let gw = build_gateway(gemini_config(_s.port));
    ...
    println!(
        "PASS S1 | provider=gemini | runtime={}ms | tokens_in={}",
        runtime.as_millis(),
...
fn s2_streaming_chat() {
    ...
    for event in stream.iter() {
        match event {
            ...
            Ok(browseros_llm::streaming::LlStreamEvent::Error(e)) => panic!("stream error: {e}"),
    ...
    println!("PASS S2 | provider=gemini | chunks={}", chunks.len());
}
...
fn s3_tool_calling() {
    ...
    match &resp.message.content {
        LlContent::ToolCall { name, .. } => {
            ...
            println!("PASS S3 | provider=gemini | tool={}", name);
        }
...
fn s_embedding() {
    ...
    let result = gw.embed(req);
    match result {
        Err(e) => {
            let msg = e.to_string().to_lowercase();
            assert!(
                msg.contains("not yet supported") || msg.contains("not supported"),
                "got: {e}"
            );
            println!("PASS embed | provider=gemini | result=not_supported (expected)");
        }
        Ok(resp) => {
            panic!(
                "Gemini embed should fail with ConfigurationError, got success with dims={}",
                resp.dimensions
            );
        }
    }
...
fn s6_timeout_handling() {
    let s = MockServer::start("gemini");
    let mut cfg = gemini_config(s.port);
    cfg.timeout_secs = 1;
...
fn s8_malformed_response() {
    let s = MockServer::start("gemini");
    let gw = build_gateway(gemini_config(s.port));
    ...
    println!("PASS S8 | provider=gemini | error=malformed_response");
}
...
fn s9_rate_limiting() {
    ...
    println!("PASS S9 | provider=gemini | error=rate_limited");
}
...
fn s10_auth_failure() {
    ...
    println!("PASS S10 | provider=gemini | error=auth_failure");
}
...
fn s11_cache_hit() {
    ...
    println!("PASS S11 | provider=gemini | cache_hit=true | calls={}", c2);
}
...
fn s13_cost_tracking() {
    ...
    println!(
        "PASS S13 | provider=gemini | tokens_in={} | cost_cents={:.4}",
...
fn s14_health_reports() {
    ...
    let gem = health.iter().find(|h| h.provider_id == "gemini");
    assert!(gem.is_some(), "health should report gemini");
    println!("PASS S14 | provider=gemini | health_reported=true");
}
